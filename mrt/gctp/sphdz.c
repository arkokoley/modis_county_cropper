...
PURPOSE:	This function assigns values to the semimajor axis, semiminor
		...
		If the spheroid code is zero or greater, the semimajor and
		semiminor axis are defined by the spheroid code, listed below
		in the array assignment. If parm[0] is the Modis sphere
                (6371007.181) then the radius is set to parm[0]. Otherwise
                the radius is set to 6370997.0.
...
----------              ----
T. Mittan	      MARCH, 1993
G. Schmidt            Nov, 2002     Changed to support Modis sphere values
                                    when datums are specified, rather than
                                    defaulting to sphere 19.
...
if (isph < 0)
   ...
   /* The spheroid code is to be used to assign the axis
    ---------------------------------------------------*/
...
/* Use the spheroid code to assign the semi-major and semi-minor axis
  ...
  /* Set the semi-major and semi-minor values
   -----------------------------------------*/
  *r_major = major[jsph];
  *r_minor = minor[jsph];

  /* Check for the Modis sphere value as the radius. If the radius is
     close to the Modis sphere value then use the radius specified in
     the projection parameters. Otherwise default to use sphere 19. */
  if (fabs (parm[0] - major[31]) < 0.000001)
    {
    *radius = parm[0];
    }
  else
